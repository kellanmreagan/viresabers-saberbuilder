...
await page.screenshot({ path: new URL("50-rook-fang.png", outDir).pathname });

const chip = await page.$eval(".placeholder-chip", (el) => el.textContent.trim());
const badges = await page.evaluate(() => ({
  rook: [...document.querySelectorAll(".card")].some(
    (el) => el.querySelector("strong")?.textContent.includes("Rook Switch") && el.textContent.includes("Real 3D")
  ),
}));
console.log(JSON.stringify({ chip, badges, errors }, null, 2));
await browser.close();
if (!chip.includes("Rook switch") || !chip.includes("Fang emitter")) process.exit(1);
