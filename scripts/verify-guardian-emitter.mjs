...
await searchAndClick("Guardian Emitter");
await new Promise((r) => setTimeout(r, 2500));
await page.screenshot({ path: new URL("49-guardian-emitter.png", outDir).pathname });

const badge = await page.evaluate(() =>
  [...document.querySelectorAll(".card")]
    .filter((el) => el.querySelector("strong")?.textContent.includes("Guardian Emitter"))
...
await browser.close();
if (!chip.includes("Guardian emitter")) process.exit(1);
